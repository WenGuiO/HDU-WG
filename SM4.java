...
public class SM4 {
    ...
    private static final int [] CK = {
                ...
                0xA0A7B0B5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB021D,
     ...
     * @return 128位字符串
     */
    private static byte[] preHandle(String ori){
        int length = ori.length();
        if(length > 16){
            System.out.println("明文/秘钥 须在16个字节以内 !");
            System.exit(0);
        }
     ...
     * @return  字符字符串
     */
    private static String byteToString(byte[] ori) {
        return new String(ori, 0, len, StandardCharsets.UTF_8);
    }
    ...
    public static void test() {
        ...
        //记录明文长度
        len = p.length();
        byte[] plaintext = preHandle(p);

        System.out.print("请输入密钥(16以内): ");
        byte[] key = preHandle(sc.next());
        //换行
        System.out.println();
        //加密
        byte[] cipher = encrypt(plaintext, key);
        System.out.println("加密结果为: " + byteToString(cipher));
        //解密
        byte[] decryptText = decrypt(cipher, key);
        System.out.println("解密结果为: " + byteToString(decryptText));
    }
